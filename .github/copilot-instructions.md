...
Note: adding this note and snapshot reduces accidental regressions but does not technically enforce immutability — use branch protections, code reviews, and emulator checks to guarantee stability.
 ...
 You can run the emulator and capture crashes by running `cd ~/Documents/VFRStopwatch/VFRStopWatch && "/Users/matiaszayamendez/Library/Application Support/Garmin/ConnectIQ/Sdks/connectiq-sdk-mac-9.1.0-2026-03-09-6a872a80b/bin/monkeydo" bin/VFRStopWatch.prg fenix7pro`. Please run it after eacxh change to see if everything works as expected. If you encounter a crash, please investigate the stack trace and fix the issue before proceeding with further changes. Always confirm the build succeeds and the app launches cleanly in the emulator before making additional edits. Also open "/Users/matiaszayamendez/Library/Application Support/Garmin/ConnectIQ/Sdks/connectiq-sdk-mac-9.1.0-2026-03-09-6a872a80b/bin/ConnectIQ.app" && sleep 8 && cd ~/Documents/VFRStopwatch/VFRStopWatch && "/Users/matiaszayamendez/Library/Application Support/Garmin/ConnectIQ/Sdks/connectiq-sdk-mac-9.1.0-2026-03-09-6a872a80b/bin/monkeydo" bin/VFRStopWatch.prg fenix7pro 2>&1 | tee /tmp/vfr_crash.log; echo "EXIT:$?" to capture a full crash log with timestamps and exit code. A clean run exits 0 with only VFRComms/backup messages; a crash prints a stack trace with "UnhandledExceptionError" or "UnexpectedTypeException" before exit. If you see "Unable to connect to simulator" from `monkeydo`, it means the ConnectIQ.app is not yet running; always open it and wait at least 5–6 seconds before invoking `monkeydo` — a reported "crash on launch" may actually be this connection failure, not a real runtime crash.